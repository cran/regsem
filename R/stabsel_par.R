...
stabsel_par<-function(data,
  ...
  if (det.range==TRUE){#use a prestep to determine lambda range
    lam<-det_range_par(data,model,times,n.lambda=detr.nlambda,jump=jump,type=type,pars_pen=pars_pen,...)
    lb<-lam$lb;ub<-lam$ub
    #rtn$det.range<-lam
  }else{
    lb<-from;ub<-to
  ...
  for (k in 1:nlambda){
    pars<-parSapply(cl,1:n.boot,function(i,nsize,data,model,Lam,k,n.pen,pars.pen,...){
      ids = sample(1:nsize,nsize,replace=T)
      datasub.ss <- data[ids,]
      data.hold.ss<-data[-unique(ids),]

      try(est_model_ss <- sem(model, data = datasub.ss))
      try(regsem.out.ss <- regsem(est_model_ss,lambda = Lam[k],pars_pen = pars.pen,...))
      coeff<-rep(NA,n.pen)
      try(coeff<-regsem.out.ss$coefficients[pars.pen])
      return(coeff)},nsize,data,model,Lam,k,n.pen,pars.pen,...)
    pars.ss.1<-t(pars)!=0
